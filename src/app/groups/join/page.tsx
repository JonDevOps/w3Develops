...
export default function JoinGroupPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) {
        toast({ variant: "destructive", title: "Not Authenticated", description: "You must be logged in to join a group." });
        router.push('/login');
        return;
    }
    if (!topic || !commitment) {
    ...
    <div className="max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Find a group that matches your interests and availability. We'll find a match for you or create a new group.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleJoin} className="grid gap-6">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Finding Group...' : 'Find and Join Group'}
            </Button>
          </form>
        </CardContent>
      </Card>
